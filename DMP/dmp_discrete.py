...
#%% define discrete dmp
class dmp_discrete():
    def __init__(self, n_dmps=1, n_bfs=100, dt=0, alpha_y=None, beta_y=None, decay=True, decay_c= 3/4, **kwargs):
    ...
    def learning(self, y_demo, plot=False):
        ...
        if plot is True:
            ...
            print((psi_track*self.w[0]).shape)
            print(np.sum(psi_track * self.w[0], axis=1).shape)
    ...
    def reproduce(self, tau=None, initial=None, goal=None):
        ...
        # reset initial and goal
        self.reset_initial_and_goal(initial, goal)
        ...
        for t in range(timesteps):
            y_reproduce[t], dy_reproduce[t], ddy_reproduce[t], _ = self.step(tau=tau)
        
        return y_reproduce, dy_reproduce, ddy_reproduce

    def step(self, a=None, tau=None):
        # run canonical system
        if tau == None:
            tau = self.tau
        x = self.cs.step_discrete(tau)
        if type(a) ==type(None):
            a = np.zeros(self.n_dmps)
        ...
        for d in range(self.n_dmps):
            ...
            if self.decay:
                action_term = a[d] * np.exp(-1/self.decay_c/(1+np.log(x))**2) / np.exp(-1/self.decay_c)
            else:
                action_term = a[d]
            f = k*(np.dot(psi, self.w[d])*x*k2 / np.sum(psi)) - k*(self.goal[d] - self.y0[d])*x + action_term
            # print(f)
    ...
    def reset_initial_and_goal(self, initial=None, goal=None):
        # set initial state
